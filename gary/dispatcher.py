...
from gary.util.util import WorkerPool
...
class MessageDispatcher(object):
    def __init__(self, slackclient, plugins, errors_to):
        ...
        alias_regex = ''
        if getattr(config, 'ALIASES', None):
            logger.info('using aliases %s', config.ALIASES)
            alias_regex = '|(?P<alias>{})'.format('|'.join([re.escape(s) for s in config.ALIASES.split(',')]))
        ...
        for func, args in self._plugins.get_plugins(category, msg.get('text', None)):
            if func:
                ...
                try:
                    ...
                    logger.exception(
                        'failed to handle message %s with plugin "%s"', msg['text'], func.__name__)
                    reply = u'[{}] I had a problem handling "{}"\n'.format(func.__name__, msg['text'])
    ...
    def filter_text(self, msg):
        ...
        if channel[0] == 'C' or channel[0] == 'G':
            ...
            logger.debug('got an @ message: %s', text)
            msg['text'] = text
        else:
            if m:
